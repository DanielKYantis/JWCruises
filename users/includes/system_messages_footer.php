...
(function(){
  ...
  /**
   * Correctly sanitizes HTML and formats it for display.
   * This is the key function that was fixed.
   */
  function sanitizeAndFormat(html) {
    // Step 1: Replace desired tags with a unique placeholder
    let temp = html.replace(/<\s*li[^>]*>(.*?)<\s*\/\s*li\s*>/gi, `$1${USERSPICE_BREAK}`);
    temp = temp.replace(/<\s*br\s*\/?>/gi, USERSPICE_BREAK);
    
    // Step 2: Use DOMParser to safely strip all other HTML tags
    const doc = new DOMParser().parseFromString(temp, 'text/html');
    const safeText = doc.body.textContent || "";
    
    // Step 3: Return the safe text with placeholders ready for node building.
    return safeText.substring(0, MAX_MESSAGE_LENGTH);
  }
  
  function buildToastBodyContent(bodyElement, message) {
    const safeMessage = sanitizeAndFormat(message);
    const parts = safeMessage.split(USERSPICE_BREAK);
    
    parts.forEach((part, index) => {
      // Create a text node for each part of the string
      bodyElement.appendChild(document.createTextNode(part.trim()));
      
      // If there are more parts, add a <br> element
